...
def upgrade() -> None:
    ...
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('actor_type', sa.Enum('USER', 'SYSTEM', name='audit_actor_type', native_enum=False), nullable=False),
    ...
    op.create_index('ix_audit_log_action_created', 'audit_log', ['action_type', 'created_at'], unique=False)
    ...
    op.create_index(op.f('ix_persons_profile_id'), 'persons', ['profile_id'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.CheckConstraint("name_type IN ('PRIMARY', 'BIRTH', 'ALTERNATIVE')", name='ck_person_names_name_type'),
    sa.ForeignKeyConstraint(['person_id'], ['persons.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_person_names_person_id'), 'person_names', ['person_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_person_names_person_id'), table_name='person_names')
    ...
    op.drop_index(op.f('ix_persons_profile_id'), table_name='persons')
